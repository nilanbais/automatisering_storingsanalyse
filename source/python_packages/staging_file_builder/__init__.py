...
staging_file_columns2 = ['werkorder', 'status', 'rapport datum', 'werkorder beschrijving',
                         ...
                         'probleem code', 'beschrijving probleem', 'oorzaak code', 'beschrijving oorzaak', 'oplossing code',
...
class StagingFileBuilder:
    ...
    _ld_map_path = "resources/information_mapping/location_description_mapping/location_description_map_coentunnel-tracé.json"  # location_description_map
    _default_file_name = f"{str(dt.now().date()).replace('-', '')}_{str((int(dt.now().time().hour) + 2))}_{str(dt.now().time().minute)}_staging_file.xlsx"  # hours + 2 to compensate the timezone (always a 2 hour diff)
    ...
    def read_sf_data(self) -> json:
        with open(f"data/raw_database_data/{self.input_file_name}", 'r') as input_file:
    ...
    def build_base_df(self) -> str:
        ...
        data_df = pd.DataFrame.from_dict(json.loads(self.raw_data))  # self.raw_data has dtype string
    ...
    def save_staging_file(self, export_path: str = None) -> str:
        ...
        export_path = '' if export_path is None else export_path

        letters = list(string.ascii_uppercase)

        # Setting up the writer
        writer = pd.ExcelWriter(os.path.join(export_path, filename), engine='xlsxwriter')
