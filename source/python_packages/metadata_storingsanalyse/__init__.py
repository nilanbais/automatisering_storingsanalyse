...
Functies/Modules die geschreven moeten worden, zijn:
    ...
    -   [x] methode voor het filteren van meldingen of storingen op een te kiezen di nummer
    -   [x] methode voor het updaten en opslaan van de nieuw gegenereerde metadata
"""
...
from source.python_packages.prepnplot import *
...
class MetadataStoringsAnalyse:
    ...
    _default_file_name_backup_version = "metadata_{0}_version_{1}_{2}.json"  # 0 = project, 1 = quarter, 2 = year
    ...
    def __init__(self, project):
        ...
        self.old_meta = None  # set by self.get_all_data()
        self.unsaved_updated_meta = None  # set by self.update_meta()
        """
        Addition to above attributes
        The two attributes above represent the meta that is read - the old metadata, and the meta that is 
        updated - the unsaved_updated meta.
        """
        self.get_all_data()
        
        self.unique_di_numbers = self.contract_info()["aanwezige_deelinstallaties"]
        self.tijdsregistratie = self.contract_info()["tijdsregistratie"]  # true or false
        
        self.quarter_sequence = PrepNPlot().quarter_sequence

    def _get_filepath(self) -> str:
        _filename = [MetadataStoringsAnalyse._filepath_dict[key]
                     for key in MetadataStoringsAnalyse._filepath_dict.keys()
                     if self.project.lower() in key.lower()]

        if len(_filename) > 1:
            raise ValueError(f"Found too many files using the given project {self.project}. \nResults: {_filename} ")
...
#         rel_filepath = "data\\metadata\\" + self.filename
    ...
    def get_all_data(self) -> DataFrame:
        self.old_meta = pd.json_normalize(data=self._read_json(), max_level=0)
        return self.old_meta

    def startdate(self) -> str:
        return self.old_meta["start_datum"][0].replace('_', '-')

    def contract_info(self) -> dict:
        return self.old_meta["contract_info"][0]

    def meldingen(self) -> dict:
        return self.old_meta["meldingen"][0]

    def storingen(self) -> dict:
#         meta = self.get_all_data()
        return self.old_meta["storingen"][0]

    # todo: toevoegen aan documentatie + de aanpassing in de objectstructuur van de metadata documenteren
    def poo_data(self) -> dict:
        return self.old_meta["poo_codes"][0]
    ...
    def get_di_set_difference(self, staging_file_di_numbers: list) -> set:
        sf_di_set = set(staging_file_di_numbers)
        diff = sf_di_set.difference(set(self.unique_di_numbers))
        return diff
    
    # todo: toevoegen aan documentatie
    def update_aanwezige_deelinstallaties(self, staging_file_di_numbers: list) -> dict:
        diff = self.get_di_set_difference(staging_file_di_numbers)
        new_list_uniques = self.unique_di_numbers.copy()
        if len(diff) > 0:
            for x in diff:
                new_list_uniques.append(x)
                
        return new_list_uniques
    
    # todo: toevoegen in documentatie
    def update_meta(self, staging_file_data: DataFrame) -> None:
        list_di_numbers = list(staging_file_data['sbs'].unique())
        # updating contract_info|aanwezige_deelinstallatie
        updated_di_numbers = self.update_aanwezige_deelinstallaties(staging_file_di_numbers=list_di_numbers)
        updated_contract_info = self.contract_info()
        updated_contract_info["aanwezige_deelinstallaties"] = list(updated_di_numbers)
        # updating poo_codes
        ...
        self.unsaved_updated_meta = {"project": str(self.project),
                                     "start_datum": str(self.startdate()),
                                     "contract_info": updated_contract_info,
                                     "poo_codes": updated_poo_data,
                                     "meldingen": updated_meldingen,
                                     "storingen": updated_storingen}
    
    # todo: toevoegen aan documentatie 
    def transform_structure_old_meta(self) -> dict:
        new_struc = {"project": str(self.project),
                     "start_datum": str(self.startdate()),
                     "contract_info": self.contract_info(),
                     "poo_codes": self.poo_data(),
                     "meldingen": self.meldingen(),
                     "storingen": self.storingen()}
        return new_struc
        
    # todo: toevoegen aan documentatie    
    def save_backup_meta(self, path_to_folder: str = "") -> None:
        prev_quarter = self.quarter_sequence.get_prev_val(self._quarter)
        filename = MetadataStoringsAnalyse._default_file_name_backup_version.format(self.project, 
                                                                                    prev_quarter, 
                                                                                    self._year)
        with open(os.path.join(path_to_folder, filename), 'w') as backup_file:
            json.dump(self.transform_structure_old_meta(), backup_file)

    # todo: toevoegen aan documentatie            
    def save_updated_meta(self, path_to_folder: str = "") -> None:
        with open(os.path.join(path_to_folder, self.filename), 'w') as new_meta_file:
            json.dump(self.unsaved_updated_meta, new_meta_file)
