...
from source.python_packages.staging_file_builder import *
from source.python_packages.metadata_storingsanalyse import MetadataStoringsAnalyse
from source.python_packages.query_maximo_database import QueryMaximoDatabase
from source.python_packages.prepnplot import PrepNPlot
...
class StoringsAnalyse(PrepNPlot):
    ...
    _ld_map_path = "resources/information_mapping/location_description_mapping"
    ...
    def __init__(self, project: str, api_key: str, rapport_type: str, quarter: str, year: str, staging_file_name: str = None) -> None:
        ...
        self.unique_di_numbers_staging_file = None  # set by self.get_unique_di_numbers()
    ...
    """
    def _read_ld_map(self) -> DataFrame:
        """
        ...
        :return:
        """
        file_path = os.path.join(StoringsAnalyse._ld_map_path, self._get_ld_file_name())
        with open(os.path.relpath(file_path), 'r') as file:
    ...
    def save_maximo_response_data(self, export_path: str = "", filename: str = _default_file_name_maximo) -> None:
        _filename = filename
        
        with open(os.path.join(export_path, _filename), 'w') as output_file:
            json.dump(self._maximo.response_data, output_file, indent=6)
    ...
    def get_unique_di_numbers(self) -> None:
        staging_file_sbs_numbers = self.staging_file_data['sbs'].unique()
        self.unique_di_numbers_staging_file = set(staging_file_sbs_numbers)
    ...
    def save_meta(self, path_to_meta_folder: str, path_to_meta_backup_folder: str) -> None:
        self.metadata.save_backup_meta(path_to_folder=path_to_meta_backup_folder)
        self.metadata.save_updated_meta(path_to_folder=path_to_meta_folder)
